...
:::info
See [Introduction and Setup](/legacy/0.9/cli-handbook/setup) first if you haven't gone through the steps above.

Also refer to [LOC Feature Overview](/legacy/0.9/tutorials/loc-feat-overview) to learn more about LOC features, especially the hierarchy of projects, scenarios, data processes and logic.
:::
...
See [CLI Command Reference](/legacy/0.9/category/cli-command-reference) to knnow more about CLI commands.
...
Here we'll use the [Quick Start](/legacy/0.9/quickstart) tutorial as our example. First create the [`payload-json-parser`](/legacy/0.9/logic-library/payload-json-parser) generic logic (CLI will prompt you for various fields and options):
...
:::warning
CLI `v0.8.0`, during the current porting process from TypeScript to Rust, does not yet support managing the following assets from local workspace:
...
:::tip
See [Source and Version Control](/legacy/0.9/cli-handbook/source-control) to learn more about project synchronisation in CLI.
:::
