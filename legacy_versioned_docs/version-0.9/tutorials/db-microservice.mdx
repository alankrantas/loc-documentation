...
As the way we've setup the HTTP agent configuration [in another tutorial](/legacy/0.9/tutorials/http-agent-configuration#create-a-configuration), go to **Administration/Agent Configuration** and add a database configuration under one of the folders:
...
| -------------------- | ------------------------------------------------------------------------------- | ---------------------------------------------------------- | ------------- |
| **Generic logic #1** | `payload-json-parser` ([source](/legacy/0.9/logic-library/payload-json-parser)) | Parse payload to JSON object                               |               |
| **Generic logic #2** | `log-service-post-logic`                                                        | Read parsed payload and generate SQL statements/parameters |               |
| **Generic logic #3** | `database-query` ([source](/legacy/0.9/logic-library/database-query))           | Query database                                             | `comx`        |
| **Aggregator logic** | `log-service-aggregator`                                                        | Finalise service result                                    |               |

Code for two of the logic (generic #1 and #3) can be found in the [Logic Module Library](/legacy/0.9/category/logic-module-library) (the database query logic handles both select and action queries).

:::note
The `database-query` accepts the SQL statements and parameters from another logic, in this case namely `log-service-post-logic`.
:::
...
| Logic                | Name                                                                                     | Purpose                                                        | DB Config Ref |
| -------------------- | ---------------------------------------------------------------------------------------- | -------------------------------------------------------------- | ------------- |
| **Generic logic #1** | `querystring-parser` ([source](/legacy/0.9/logic-library/querystring-parser))            | Parse QueryString to object                                    |               |
| **Generic logic #2** | `log-service-get-logic`                                                                  | Read parsed QueryString and generate SQL statements/parameters |               |
| **Generic logic #3** | `database-query` ([source](/legacy/0.9/logic-library/database-query))                    | Query database and return results                              | `comx`        |
| **Aggregator logic** | `log-service-aggregator` (see [`log-service-post`](#log-service-post-post-log-messages)) | Finalise service result                                        |               |

Like the first data process, code for two of the logic (generic #1 and #3) can be found in the [Logic Module Library](/legacy/0.9/category/logic-module-library). This means the two data processes actually share 50% of the code with reusable logic modules.
...
Finally [create API routes](/legacy/0.9/tutorials/create-api-route#create-an-api-route) for both data processes:
...
> You can also test the data processes with [Single Data Process Execution](/legacy/0.9/quickstart#run-the-data-process---simple-data-process-execution).
...
The execution result we had above includes the execution metadata, however you can omit these information by turning the API routes' [encapsulation](/legacy/0.9/studio-guide/triggers) off.
