...
Logic are the core elements of a [data process](/legacy/0.9/tutorials/loc-feat-overview#data-process-and-logic). They contains the implementation of a data pipeline (which may be part of a bigger one). And the **session storage** is the thing to link them up together during a task.
...
:::info
We will discuss more about logic error handling in [Tips on Error Handling](/legacy/0.9/tutorials/error-handling-tip).
:::
...
Both `run` and `handleError` has to be exported with `export` keyword. [SDK](/legacy/0.9/category/sdk-reference) is imported for accessing agents and related types.
...
Logic can read and write session data using the [Session Storage Agent](/legacy/0.9/sdk-reference/session). We will use a simple example as demostration.
...
| Aggregator logic     | `result-aggregator` ([source](/legacy/0.9/logic-library/result-aggregator))     | Finalise result                                                    |
...
:::info
The first logic (payload parser) and the aggregator logic in this section are the same as the [previous version](/legacy/0.9/tutorials/logic-and-session#code-walkthrough).
:::
