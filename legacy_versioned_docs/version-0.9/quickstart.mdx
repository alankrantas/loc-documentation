...
First login LOC [Studio](/legacy/0.9/category/studio-guide):
...
:::info
See [User Management](/legacy/0.9/studio-guide/user-management) for how to setup a new account in LOC. You may need to contact your LOC Studio administrator.
:::
...
> You can also find the source code [here](/legacy/0.9/logic-library/result-aggregator).
...
:::info
JavaScript with JSDoc annotations is still JavaScript; however it can enable type checking intellisense in Studio and local editors like VS Code. See [CLI Handbook](/legacy/0.9/category/cli-handbook) for how to develop locally.
:::
...
:::tip
For aggregator logic, you must select the logic type as **Aggregator** in the logic creation window. Otherwise you wouldn't be able to add it into your data process.
:::
...
We can execution the data process with so-called **simple data process execution**, which is in fact using a mock [API route](/legacy/0.9/tutorials/create-api-route) trigger (simulating an incoming HTTP POST request).
...
We can see the execution ID, task ID as well as the finalised result returned by the aggregator logic (by clicking the _Preview JSON_ button):
...
:::info
What would happen if you execute the data process right away with _no_ payload? Our `payload-json-parser` logic is designed to skip parsing error, and `greet` logic would thus fall back to `"World"` as the default name (outputing `"Hello, World!"`).
:::

:::tip
For how to properly invoke a data process with an API route trigger (HTTP endpoint), see [Create and Use an API Route Trigger](/legacy/0.9/tutorials/create-api-route).
:::
...
1. First proceed to [Tutorials](/legacy/0.9/category/tutorials), which offers basic examples on specific topics and activities for you to try in LOC Studio.
2. The tutorials also point you to various pages in [Studio Guide](/legacy/0.9/category/studio-guide) and [SDK Reference](/legacy/0.9/category/sdk-reference) for additional details.
3. If you are a developer, you'll need to learn about CLI, which can do more and makes debugging easier - [CLI Handbook](/legacy/0.9/category/cli-handbook) plus SDK Reference have everything you need.
