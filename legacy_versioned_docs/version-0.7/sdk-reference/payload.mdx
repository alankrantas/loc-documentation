...
Payload is the data sent by triggers as initial input and is accessible from the [context](/legacy/0.7/sdk-reference/context-and-task) object. Currently there are two payload types:
...
The payload would _only_ be lazy-loaded after calling `payload()` in a logic. The payload is a union type, which means it can be one of the following types at runtime:
...
<details>
    <summary><b>Tip</b> on Using <b>Type Guards</b> or <b>JSDoc Annotations</b> for Safer Payload Typing</summary>

#### TypeScript Type Guards (Type Narrowing)

What we did above in the TypeScript example is the so-called [**type assertion**](https://www.typescriptlang.org/docs/handbook/2/everyday-types.html#type-assertions), which means you take the responsibility of type safty and tell the TypeScript compiler to shut up. This is handy when you are absolutely sure what kind of payload is expected in one logic.

However, type assertions cannot prevent `payload` getting a wrong type of object at runtime and may cause errors. While we still use type assertions in most of our examples for the sake of demostration, you should _use it at your own risk_.

A safer way is to use a [**type guard**](https://www.typescriptlang.org/docs/handbook/2/narrowing.html) to narrow down the union type:
...
| `request`                   | `HttpRequest` (see below)                                            | Request content                   |
...
:::note
`URLSearchParams` is part of the [Web API](https://developer.mozilla.org/en-US/docs/Web/API) which is supported in LOC's logic runtime.
:::
...
| ----------------------- | --------------------------------------------------------------- | ------------------------------- |
| `clientIdentityContext` | `DataSourceIdentityContext` (see [HTTP payload](#http-payload)) | MQ client permanent ID and name |
| `subscriber`            | `Subscriber`                                                    | MQ subscriber                   |
| `data`                  | `number[]`                                                      | MQ data                         |

`Subscriber` type describe the message queue source:
