...
This is the follow-up of [Emit and Inspect Events](/legacy/0.8/tutorials/emit-and-inspect-event), in which we've learned the basics of emitting events.
...
This new data process will query the `EventPolicyRequest` events emitted in the [previous tutorial](/legacy/0.8/tutorials/emit-and-inspect-event), extract the request ID and policy code, then assign to the agent who is responsible for a specific type of policy:
