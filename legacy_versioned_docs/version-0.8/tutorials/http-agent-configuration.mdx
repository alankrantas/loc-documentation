...
LOC provides [agents](/legacy/0.8/category/sdk-reference) for operating not only internal data sources, but also external ones like HTTP endpoints, databases, FTP servers and SMTP servers. These is what makes LOC truely useful for building data pipelines across your organization.
...
For this tutorial, we'll use the [HTTP agent](/legacy/0.8/sdk-reference/http) to invoke `https://httpbin.org/anything`. This is a public API that simply _returns anything_ you've sent to it, which is ideal for testing purposes.

In Studio, go to **Administration** then click the panel [Agent Configuration](/legacy/0.8/studio-guide/agent-configuration). Right-click on the Unit to create a folder with a name:
...
A configuration has to be manually added to a logic (which has to be linked in a data process) to be accessible. The [HTTP Agent](/legacy/0.8/sdk-reference/http) would be able to load the configuration and returns a working HTTP client object.
...
:::info
The **Variable** panel above is for setting up logic environment variables. See [Logic Variable Agent](/legacy/0.8/sdk-reference/logic-var) for how to access them in logic.
:::
...
:::tip
HTTP Agent loads configuration from its reference name, _not_ the configuration name.
:::
...
Now you can try to invoke the data process with a JSON payload, either using [single data process execution](/legacy/0.8/quickstart#run-the-data-process---simple-data-process-execution) or via an [API route](/legacy/0.8/tutorials/create-api-route).
