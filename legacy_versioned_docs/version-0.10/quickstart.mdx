...
```mermaid
flowchart LR;
    n1((JSON<br>payload))-->|Invoke|n2;
    n2{{Hello World<br>data process}}-->|Respond|n3;
    n3((JSON<br>response));
```
...
First login LOC [Studio](/legacy/0.10/category/studio-guide):
...
:::info
See [User Management](/legacy/0.10/studio-guide/user-management) for how to setup a new account in LOC. You may need to contact your LOC Studio administrator.
:::
...
_Logic_ are modularised, reusable code components to be linked to data processes, which are created and managed seperatly with [projects, scenarios and data processes](/legacy/0.10/tutorials/loc-feat-overview). Each data process should links with _at least one_ **generic logic** and _only one_ **aggregator logic**. Generic logic will be executed by order and finally run the aggregator.
...
flowchart TD;
    ...
    n3["Payload JSON Parser<br>(generic logic)"]-->|parsed JSON|n4;
    n4["Greeting<br>(generic logic)"]-->|greeting message|n5;
    end
    n5["Result Aggregator<br>(aggregator logic)"]-->|finalised result|n6((JSON response));
```
...
> You can also find the source code [here](/legacy/0.10/logic-library/result-aggregator).
...
:::info
JavaScript with JSDoc annotations is still JavaScript; however it can enable type checking intellisense in Studio and local editors like VS Code. See [CLI Handbook](/legacy/0.10/category/cli-handbook) for how to develop locally.
:::
...
:::note
_Revisions_ of a logic represent its versions and editing history, which also ensures that your data process won't break if they depends on older versions.
:::
...
<img src={require("./quickstart/04-1.png").default} alt="04-1" />
...
Then right click on the project to create a _scenario_ under it (again, give it a name and optional descriptions).
...
<div className="center-padded-sm">
    <img src={require("./quickstart/13-1.png").default} alt="13-1" />
</div>
...
We can execution the data process with **manual trigger**, which is in fact using a mocked [API route](/legacy/0.10/tutorials/create-api-route) trigger simulating an incoming HTTP GET request.
...
We can see the execution ID, task ID as well as the finalised result ([**task result**](/legacy/0.10/tutorials/loc-feat-overview#triggers-tasks-and-executions)) returned by the aggregator logic (by clicking the _Preview JSON_ button):
...
:::info
What would happen if you execute the data process right away with _no_ payload? The data process will, in fact, receive a payload with _empty request body_, and our `Payload JSON Parser` logic is designed to skip parsing error. Since the next logic `Greet` would read `null` while trying to get the `parsed` session variable, it will fall back to the default name `"World"` and return the message `"Hello, World!"`.
...
-   With an API route: [Create and Use an API Route Trigger](/legacy/0.10/tutorials/create-api-route)
-   With a schedule: [Create an ETL Pipeline with Schedule](/legacy/0.10/tutorials/create-schedule).
    :::
...
1. First proceed to [Tutorials](/legacy/0.10/category/tutorials), which offers basic examples on specific topics and activities for you to try in LOC Studio.
2. The tutorials also point you to various pages in [Studio Guide](/legacy/0.10/category/studio-guide) and [SDK Reference](/legacy/0.10/category/sdk-reference) for additional details.
3. If you are a developer, you'll need to learn about CLI, which can do more and makes debugging easier - [CLI Handbook](/legacy/0.10/category/cli-handbook) plus SDK Reference have everything you need.
