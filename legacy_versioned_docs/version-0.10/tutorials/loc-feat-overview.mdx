...
| **Resources** | Allocated hardware resources (CPU cores and memory) for LOC runtime  |

:::note
The available resources for LOC runtime - which executes the data processes - is defined in your license based on your purchased options. The rest of LOC requires additional infrastructure resources to run its services, which has nothing to do with the runtime resources.
:::
...
-   [Logic and Session](/legacy/0.10/tutorials/logic-and-session)
-   [Tips on Error Handling](/legacy/0.10/tutorials/error-handling-tip)
...
For demostration purposes we will still mostly use cloud logic in our [tutorials](/legacy/0.10/category/tutorials). See [CLI Handbook](/legacy/0.10/category/cli-handbook) to learn more about CLI.
...
You can find full detailed interface documentation at [SDK Reference](/legacy/0.10/category/sdk-reference).
...
-   [Send HTTP Requests Using Agent Configuration](/legacy/0.10/tutorials/http-agent-configuration)
-   [Create a Data-Driven Microservice with Database](/legacy/0.10/tutorials/db-microservice)
...
| **Schedule**                  | LOC scheduler          | No              | None (execution history only) |

A data process linked to a trigger asset is referred as a **trigger task**. A trigger can have multiple trigger tasks.

When a trigger asset is invoked, the LOC runtime will start an **execution**. Each data processes run in the execution is called a **task**. All tasks would be run simultaneously and would receive the same payload from the trigger if there is one.
...
Both execution and task would generate their unique IDs, and the execution result/logs would be stored as **execution history**.

:::note

#### Execution Result and Task Result

After the execution, a trigger would return a **execution result**, which is the combined output of _all task results_. A result in JSON format would look like this:
...
```mermaid
flowchart TD;
    n1[API route]-->|payload|n4;
    n2[MQ trigger]-->|payload|n4;
    n3[Schedule]-->n4;
...
-   [Create and Use an API Route Trigger](/legacy/0.10/tutorials/create-api-route)

:::info
LOC Studio and CLI only supports managing API routes.

MQ and schedule were once available to manage via CLI in `v0.7.0`, however since the new CLI is currently being ported from TypeScript to Rust, these functionalities will be available again in later releases.
:::

## Events

**Events** or _data events_ are metadata emitted by users with [Event Agent](/legacy/0.10/sdk-reference/eventstore) to the LOC event store, which is built on Elasticsearch.
