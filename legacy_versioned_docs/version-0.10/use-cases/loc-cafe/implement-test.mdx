...
:::tip
We assume that you would be using [Studio](/legacy/0.10/category/studio-guide) for the tutorial. See the [CLI Handbook](/legacy/0.10/category/cli-handbook) for how to create LOC assets in local editors like VS Code.
:::

## Create Agent Configuration

> See: [Create a Configuration](/legacy/0.10/tutorials/http-agent-configuration#create-a-configuration)
...
> See: [Create a Generic Logic](/legacy/0.10/quickstart#create-a-generic-logic)

Create all [logic](/legacy/0.10/use-cases/loc-cafe/system-design#logic-design) described in the previous page.

## Create Project, Scenario and Data Processes

> See: [Build the Data Process](/legacy/0.10/quickstart#build-the-data-process)

Create a project and a scenario if you don't jave any, then create the two [data processes](/legacy/0.10/use-cases/loc-cafe/system-design#data-process-design) described in the previous page from the logic above.

### Set the Agent Configuration

> See: [Add a Configuration to a Linked Logic](/legacy/0.10/tutorials/http-agent-configuration#add-a-configuration-to-a-linked-logic)
...
> See: [Create an API Route](/legacy/0.10/tutorials/create-api-route)

Create the two [API routes](/legacy/0.10/use-cases/loc-cafe/system-design#api-routes) described in the previous page and link the corresponding data processes.
...
> See: [Invoke a Data Process with API Route](/legacy/0.10/tutorials/create-api-route#invoke-a-data-process-with-api-route)

With all the assets in place, you can invoke the data processes with one of the following approaches:

1. Invoke API route `loc-cafe-place-order` with a HTTP POST request and the [JSON payload](/legacy/0.10/use-cases/loc-cafe/system-design#order-payload-example) using a HTTP client.
2. Use [manual trigger](/legacy/0.10/quickstart#manually-trigger-the-data-process) to run the data process `placeOrderDP`.
...
After executing the use case, you can inspect the events in [Data Discovery](/legacy/0.10/studio-guide/data-discovery) (by setting the time range filter to a few minutes ago):
