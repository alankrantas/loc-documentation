...
| ------------- | ---------------------------------- | --------------------------------------------- |
| API route     | `{ http: HttpPayload }`            | [HttpPayload](#http-payload)                  |
| Message queue | `{ messageQueue: MessagePayload }` | [MessageQueuePayload](#message-queue-payload) |
| Event         | `{ event: EventPayload }`          | Event payload (not yet implemented)           |

The schedule triggers have no payload and only returns an empty object (see [below](#schedule-payload)).
...
| `subscriber`            | [`Subscriber`](/legacy/0.10/sdk-reference/payload#subscriber-types) | MQ subscriber                   |
| `data`                  | `number[]`                                                          | MQ data                         |
...
Schedule triggers _do not have payload_. The returned payload object would simply be `{ event: {} }`. This may be changed in future LOC releases.
