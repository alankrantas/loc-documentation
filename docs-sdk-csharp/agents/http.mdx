...
if (response.IsSuccessStatusCode)
{
    ...
    if (response.Headers.Contains("Authorization")){
        string authorization = response.Headers.GetValues("Authorization").First().ToString();
    }
    if (response.Headers.Contains("Content-Type")){
        string contentType = response.Headers.GetValues("Content-Type").First().ToString();
    }
...
You can parse the body string into a JSON object (see the [examples](/sdk-csharp/payload/http-payload#examples) in parsing trigger HTTP payload).
